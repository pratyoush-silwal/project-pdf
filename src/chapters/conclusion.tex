...
\item
  ...
  \item 
Faster Training Using Alternative Simulators: The current training pipeline relies on SUMO, which, while realistic, is computationally intensive and limits the speed at which the agent can be trained across large numbers of episodes. Future work could explore lightweight traffic simulation platforms such as CityFlow, which is specifically designed for large-scale reinforcement learning research and can simulate traffic significantly faster than SUMO by sacrificing some physical realism.
\item
