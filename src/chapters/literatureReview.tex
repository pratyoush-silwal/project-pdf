...
\noindent The literature for this project was reviewed with two questions in mind. First, what conventional signal-control methods provide a reasonable baseline for an isolated intersection? Second, what parts of reinforcement learning are actually useful for a single-junction DQN implementation? This matters because the project should not simply add a neural network to a traffic simulation. It also needs a defensible baseline, a state representation that captures the Thapathali traffic condition, and a reward function that matches the evaluation metrics. The local Thapathali case study provides the field context by reporting observed peak-hour volumes, delay-related indicators, queue behaviour, and level of service for the existing operation \cite{maharjan2023performance}.
...
For a single-intersection project, the key design choices are easy to list but difficult to tune in practice: the observed state, the allowed actions, and the reward. Prior work commonly uses lane-level counts, queue lengths, waiting time, and the current signal phase as state information, while rewards are often based on delay, queue length, pressure, or throughput \cite{wei2019colight,wei2020survey}. This directly shaped the present design. The state vector combines PCU-weighted incoming loads, stopline waiting times, outgoing traffic load, and active phase information because each part describes a different control concern at Thapathali.
