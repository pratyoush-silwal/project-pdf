...
The SumoEnvironment class acts as the boundary between SUMO and the reinforcement learning code. The reset() method starts a new episode and clears the internal counters. The step(action) method applies the selected signal decision for a 15-second window, collects rewards during that window, and returns the next state with episode metrics. The \_get\_state() method extracts the 21-dimensional state vector from lane loads, stopline waiting times, outgoing PCU values, and the active phase. The \_calculate\_reward() method compares stopline PCU against outgoing PCU discharge and then adds the PCU-weighted waiting-time penalty. Signal changes are finally applied through TraCI, including the yellow transition between phases.
