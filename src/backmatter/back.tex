...
class DQNetwork(nn.Module):
    """
    Deep Q-Network.
    Input : 21-dim state [7 origin PCU | 6 stopline wait | 5 outgoing PCU | 3 one-hot]
    Output: Q-value for STAY (0) and NEXT (1)
    Architecture: 128 -> 64 -> 32 hidden layers, ReLU activations
    """
    ...
    def __init__(self, state_size, action_size):
        ...
        layers  = []
        in_size = state_size
        for out_size in config.HIDDEN_LAYERS:
...
class ReplayBuffer:
    """
    Circular experience replay memory -- capacity 10,000.
    Stores (state, action, reward, next_state, done).
    Random sampling breaks time correlation so agent learns
    from mix of all traffic patterns simultaneously.
    """
...
class DQNAgent:
    ...
    def __init__(self, state_size=config.STATE_SIZE,
        ...
        # Device selection: MPS (Apple Metal GPU) > CUDA > CPU
...
def train_step(self):
        ...
        Bellman target: reward + gamma * max(target_net(next_state))
        Returns loss value, 0.0 if buffer not full yet.
        """
...
def save(self, path):
    ...
    def load(self, path):
        ...
        print(f"  [DQN] Loaded <- {path}  (epsilon={self.epsilon:.4f})")
\end{lstlisting}
